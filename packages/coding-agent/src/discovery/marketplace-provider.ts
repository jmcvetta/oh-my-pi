/**
 * Shared Marketplace Plugin Provider Factory
 *
 * One loader implementation backs two registration surfaces:
 *   - `claude-plugins` — plugins installed through the Claude Code registry
 *     (`~/.claude/plugins/cache/`, gated by Claude `enabledPlugins` settings).
 *   - `omp-marketplace` — plugins installed through OMP's own user/project
 *     registries (`~/.omp/plugins/installed_plugins.json`) plus `--plugin-dir`
 *     roots.
 *
 * Both providers load the same six legacy capabilities (skills, rules, slash
 * commands, hooks, custom tools, MCP servers) from marketplace plugin roots.
 * The root selector on `listClaudePluginRoots` keeps their registries
 * independent, so disabling one provider no longer suppresses the other's
 * plugins.
 */
...
/** Registration options for a marketplace plugin provider. */
export interface MarketplaceProviderOptions {
	/** Capability-registry provider ID (`claude-plugins` or `omp-marketplace`). */
...
/**
 * Build the six legacy marketplace loaders bound to one provider's identity
 * and registry source.
 */
function createMarketplaceLoaders(options: MarketplaceProviderOptions) {
	const providerId = options.id;

	/**
	 * Plugin roots this provider may process for a given surface. Roots whose
	 * root `plugin.json` targets the Agent Plugins standard keep their portable
	 * components (skills, MCP) exclusive to the `agent-plugins` provider;
	 * fatally invalid Agent Plugins packages are skipped entirely.
	 *
	 * The foreign `~/.claude/plugins` tree additionally needs user-source
	 * opt-in at user scope (project roots stay available without it). OMP
	 * registry and `--plugin-dir` roots are OMP's own installs and need no
	 * foreign opt-in.
	 */
	async function allowedRoots(
		...
		const { roots, warnings } = await listClaudePluginRoots(ctx.home, ctx.cwd, options.source);
		...
		if (options.source === "claude") {
			...
			if (!userEnabled) scopedRoots = roots.filter(root => root.scope === "project");
		}
	 ...
	 */
	async function resolvePluginDir(root: ClaudePluginRoot, opts: ResolvePluginDirOptions): Promise<ResolvedPluginDir> {
		const pluginManifest = await readPluginManifest(root);
		const manifests = opts.marketplaceRootManifest
			? [pluginManifest, opts.marketplaceRootManifest]
			: [pluginManifest];
		const fallbackDir = path.join(root.path, opts.fallback);
		const configured: Array<{ entryPath: string; key: keyof ClaudePluginManifest }> = [];

		for (const manifest of manifests) {
			if (manifest === null) continue;
			for (const key of opts.manifestKeys) {
		...
		if (opts.includeFallback && !opts.marketplaceRootManifest) {
		...
		for (const { entryPath, key } of configured) {
			...
			if (!isWithinPluginRoot(root.path, resolved)) {
				warnings.push(
					`[${providerId}] Ignoring ${String(key)} path outside plugin root for ${root.id}: ${entryPath}`,
				);
				continue;
			}
	...
	async function loadRules(ctx: LoadContext): Promise<LoadResult<Rule>> {
		...
		const results = await Promise.all(
			roots.map(root =>
				loadFilesFromDir<Rule>(ctx, path.join(root.path, "rules"), providerId, root.scope, {
					extensions: ["md", "mdc"],
			...
			roots.map(async root => {
				...
				const commandResults = await Promise.all(
					commandsDirs.map(async dir => {
									...
									items: [
										{
											...
											content,
											level: root.scope,
											_source: createSourceMeta(providerId, dir, root.scope),
										},
									],
									warnings: [],
								};
						...
						return loadFilesFromDir<SlashCommand>(ctx, dir, providerId, root.scope, {
							extensions: ["md"],
							transform: (name, content, filePath, source) => {
	...
	async function loadHooks(ctx: LoadContext): Promise<LoadResult<Hook>> {
		...
		const results = await Promise.all(
			loadTasks.map(async ({ root, hookType }) => {
				...
				return loadFilesFromDir<Hook>(ctx, hooksDir, providerId, root.scope, {
	...
	async function loadTools(ctx: LoadContext): Promise<LoadResult<CustomTool>> {
		...
		const results = await Promise.all(
			roots.map(async root => {
				...
				return loadFilesFromDir<CustomTool>(ctx, toolsDir, providerId, root.scope, {
					extensions: ["ts", "js"],
					transform: (name, _content, filePath, source) => {
			...
			if (typeof pointer === "string") {
				...
				if (!isWithinPluginRoot(root.path, resolved)) {
					return {
						...
						declared: true,
						warnings: [
							`[${providerId}] Ignoring mcpServers path outside plugin root for ${root.id}: ${configured}`,
						],
					};
				}
				return {
	...
	async function loadMCPServers(ctx: LoadContext): Promise<LoadResult<MCPServer>> {
		...
		for (const root of roots) {
			...
			if (resolved.inlineServers) {
				...
				if (raw === null) {
					...
					if (resolved.declared) {
						const warning = `[${providerId}] Missing mcpServers file declared by ${root.id}: ${resolved.path}`;
				...
				try {
					...
					warnings.push(`[${providerId}] Invalid JSON in ${resolved.path}`);
					logger.warn(`[${providerId}] Invalid JSON in ${resolved.path}`);
					continue;
				}
			...
			for (const serverName in servers) {
				...
				// occasionally ship .mcp.json entries with neither, which would register a
				// useless server and surface as a connection error at runtime.
				if (typeof raw.command !== "string" && typeof raw.url !== "string") {
					warnings.push(
						`[${providerId}] Skipping MCP server "${serverName}" in ${sourcePath}: missing command or url`,
					);
					continue;
				}
				...
				if (raw.env !== undefined && !isStringRecord(raw.env)) {
					warnings.push(`[${providerId}] Skipping MCP server "${serverName}" in ${sourcePath}: malformed env`);
					continue;
				}
				...
				const server: MCPServer = {
					...
					_source: createSourceMeta(providerId, sourcePath, root.scope),
				};
				items.push(server);
			}
		}
...
/**
 * Register one marketplace plugin provider (`claude-plugins` or
 * `omp-marketplace`) against the six legacy capabilities. The loaders share
 * this module's implementation, closed over the given options.
 */
